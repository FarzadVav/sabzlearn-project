const $ = document
import { getAllUsers } from "./function/utility.js"
...
const showUsers = $.querySelector('#show-users')
...
// insert users to html element
const insertUsers = users => {
    console.log(users);
    showUsers.innerHTML = ''
    users.forEach(user => {
        showUsers.insertAdjacentHTML('beforeend', `
            <tr class="w100 flex between">
                ...
                <td class="user-delet" onclick='deleteUser(${JSON.stringify(users)}, ${JSON.stringify(user._id)})'>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path opacity="0.4"
